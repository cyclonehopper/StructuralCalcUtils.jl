...
export IbeamPlusBottomIbeam
struct IbeamPlusBottomIbeam <: DesignSections
    d::Float64
    bf::Float64
    tf::Float64
    tw::Float64
    ...
    # end
    function IbeamPlusBottomIbeam(d, bf, tf, tw, rr, d1, bf1, tf1, tw1, rr1, fy, fu, stresscode)
        ...
        # flange outstand slenderness, +Ibeam section
        bef_1 = (bf1 - tw1) / 2.0
        λef_1 = bef_1 / tf1 * sqrt(fy / 250.0)
        push!(λe, λef_1)
        push!(λe, λef_1)
        push!(λe, λef_1)
        push!(λe, λef_1)
        ...
        #rolled web, addnl Ibeam section
        push!(λe, (d1 - 2.0 * tf1) / tw1 * sqrt(fy / 250.0))
        push!(λey_axial, get_λey_axialcompression(isBoth, stresscode))
        ...
        # rolled web, UC 
        push!(λep, get_λep(isBoth, stresscode, isUniformCompression)) #HR
        push!(λey_bending, get_λey_bending(isBoth, stresscode, isUniformCompression)) # all in one of neutral axis


        new(d, bf, tf, tw, rr, d1, bf1, tf1, tw1, rr1, fy, fu, stresscode, geom, Ag, Izp, Iyp, Szp, Syp, Zzp, Zyp,
